...
public class Account {
	...
	public static Consumer<Account> add100 = (Account account)->{ account.balance+=100;};
	...
	public static Predicate<Account> checkBound = (Account a)-> lowerBound.test(a) && upperBound.test(a);
	...
	// TODO: Task3 
	// replace the null with a lambda expression
	public static AddMaker maker = (int n)-> account -> account.balance+=n;
	...
	public static int getMaxAccountID(List<Account> accounts) {
		...
		Account maxOne = accounts.stream().reduce(new Account(0, -100),
				(Account currentMax, Account current)-> {
					if(current.balance >= currentMax.balance)
						currentMax = current;
					return currentMax;
				}
				);
